...
def main():
    ...
    file_menu.add_command(label = 'открыть', command = Controller.save_as_file)
...
def right_button_menu(event):
    ...
    file_menu.add_separator()
    file_menu.add_command(label = 'Удалить контакт', command = lambda: Controller.delete_contract(rowID))
